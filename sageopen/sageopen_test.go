...
import (
	...
	"github.com/tamnd/sageopen-cli/sageopen"
)
...
func TestRecent(t *testing.T) {
	payload := papersPayload([]map[string]any{
		{
			"DOI":                    "10.1177/20503121221001",
			"title":                  []string{"Open Access Research in Social Sciences"},
			"author":                 []map[string]any{{"given": "Alice", "family": "Smith"}},
			"published":              map[string]any{"date-parts": [][]int{{2026, 6}}},
			"type":                   "journal-article",
			"container-title":        []string{"SAGE Open"},
	...
	cfg := sageopen.DefaultConfig()
	cfg.BaseURL = srv.URL
	cfg.Rate = 0

	c := sageopen.NewClient(cfg)
	...
	if papers[0].Title != "Open Access Research in Social Sciences" {
	...
	if papers[0].URL != "https://doi.org/10.1177/20503121221001" {
...
func TestSearch(t *testing.T) {
	...
	c := sageopen.NewClient(cfg)
	papers, err := c.Search(context.Background(), "machine learning", 10)
	if err != nil {
		t.Fatal(err)
	}
	if len(papers) != 1 {
