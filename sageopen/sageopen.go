// Package sageopen is the library behind the sageopen command line:
// the HTTP client, request shaping, and the typed data models for SAGE Open
...
const DefaultUserAgent = "Mozilla/5.0 (compatible; sageopen-cli/0.1; +https://github.com/tamnd/sageopen-cli)"
...
// Client talks to the CrossRef API for SAGE Open papers.
type Client struct {
	cfg        Config
...
// Recent fetches recently published SAGE Open papers.
func (c *Client) Recent(ctx context.Context, limit int) ([]Paper, error) {
	if limit <= 0 {
		limit = 20
	}
	apiURL := fmt.Sprintf("%s/members/179/works?rows=%d&sort=published&order=desc", c.cfg.BaseURL, limit)
	return c.fetch(ctx, apiURL, limit)
}

// Search searches SAGE Open papers by keyword.
func (c *Client) Search(ctx context.Context, query string, limit int) ([]Paper, error) {
	if limit <= 0 {
		limit = 20
	}
	apiURL := fmt.Sprintf(
		"%s/members/179/works?rows=%d&sort=published&order=desc&query=%s",
