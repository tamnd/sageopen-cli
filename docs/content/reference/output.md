...
describes what users get. Pick a format with `-o`, or let sageopen choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
sageopen <command> -o table   # aligned columns for reading
sageopen <command> -o jsonl   # one JSON object per line, for piping
sageopen <command> -o json    # a single JSON array
sageopen <command> -o csv     # spreadsheet friendly
sageopen <command> -o tsv     # tab-separated
sageopen <command> -o url     # just the URL column
sageopen <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
sageopen <command> --fields id,title,url
```
...
```bash
sageopen <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
sageopen <command>            # a table, because this is a terminal
sageopen <command> | wc -l    # JSONL, because this is a pipe
```
